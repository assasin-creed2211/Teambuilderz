...
app.post('/api/v1/users', verifyToken, requireRole('Admin'), validateBody(schemas.userCreate), async (req, res) => {
  try {
    ...
    // Check for duplicate name if role is Recruiter
    if (role === 'Recruiter') {
      const existingNameCheck = await pool.query(
        'SELECT id, name, email FROM users WHERE name = $1 AND role = $2',
        [name, 'Recruiter']
      );
      
      if (existingNameCheck.rows.length > 0) {
        return res.status(409).json({ 
          message: 'A recruiter with this name already exists',
          existing_recruiter: existingNameCheck.rows[0],
          suggestion: 'Please use a different name or contact an admin to resolve duplicates'
        });
      }
    }
...
app.put(
  ...
  try {
    ...
    // Check for duplicate name if updating to Recruiter role
    if (name !== undefined && role === 'Recruiter') {
      const existingNameCheck = await pool.query(
        'SELECT id, name, email FROM users WHERE name = $1 AND role = $2 AND id != $3',
        [name, 'Recruiter', userId]
      );
...
// Duplicate Recruiter Management Endpoints
